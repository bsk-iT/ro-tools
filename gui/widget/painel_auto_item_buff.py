...
class PainelAutoItemBuff(QWidget):
    def __init__(self, parent):
        ...
        self._config_layout()
        APP_CONTROLLER.updated_job.connect(self.update_item_buffs)
    ...
    def _config_layout(self):
        ...
        self.update_item_buffs(APP_CONTROLLER.job)
        APP_CONTROLLER.added_item_buff.connect(self._on_add_skill)

    def update_item_buffs(self, job: Job):
        ...
        for group, items in ITEM_BUFF_GROUP.items():
            ...
            for item in items:
                if item.id not in [a_item.id for a_item in APP_CONTROLLER.job_item_buffs]:
                    continue
                has_item = True
                vbox_item_buff.addWidget(self._build_item_inputs(job, item))
            if has_item:
    ...
    def _active_skill(self, job: Job, item: Buff, active=True):
        self.update_item_buffs(job)
        CONFIG_FILE.update_config(active, [job.id, AUTO_ITEM, ITEM_BUFF, item.id, ACTIVE])
        self.cbox_item.build_cbox()

    def _on_add_skill(self, item: Buff):
        APP_CONTROLLER.job_item_buffs.append(item)
    ...
    def _on_remove_item(self, job: Job, item: Buff):
        APP_CONTROLLER.job_item_buffs.remove(item)
    ...
    def _build_config(self, job: Job):
        ...
        city_block = CONFIG_FILE.get_value([job.id, AUTO_ITEM, ITEM_BUFF, CITY_BLOCK])
    ...
    def _update_city_block(self, state, job: Job):
        CONFIG_FILE.update_config(state.value == 2, [job.id, AUTO_ITEM, ITEM_BUFF, CITY_BLOCK])
