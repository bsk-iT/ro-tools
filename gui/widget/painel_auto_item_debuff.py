...
from service.config_file import ACTIVE, AUTO_ITEM, CONFIG_FILE, ITEM_BUFF, ITEM_DEBUFF, KEY
...
class PainelAutoItemDebuff(QWidget):
    ...
    def update_item_debuffs(self):
        ...
        for group, items in ITEM_DEBUFF_GROUP.items():
            ...
            for item in items:
                if item.id not in [a_item.id for a_item in APP_CONTROLLER.item_debuffs]:
                    continue
                has_item = True
                vbox_item_debuff.addWidget(self._build_item_inputs(item))
            if has_item:
    ...
    def _build_item_inputs(self, item: Buff):
        ...
        key_base = f"{AUTO_ITEM}:{ITEM_DEBUFF}:{item.id}:"
