...
class AutoYgg(BaseEvent):
    ...
    def __init__(self, game, name="AUTO_YGG", event_type=EventType.AUTO_POT, resource=Resource.YGG, priority=Priority.REALTIME):
        super().__init__(game, name, event_type, resource, priority)

    def check_condition(self) -> bool:
        super().check_condition()
        sp_percent = EVENT.get_config_be(Prop.SP_PERCENT, self)
        hp_percent = EVENT.get_config_be(Prop.HP_PERCENT, self)
